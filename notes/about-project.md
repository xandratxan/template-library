...
- Provide a minimal, well-structured Python library template for scientific / measurement tooling (ionizing radiation,
  metrology, physics).
- Make it easy to start a new library with best-practice layout: `src/` packaging, Hatch-based builds, Sphinx docs, and
  pytest-based tests.
- Include a small, realistic dependency set (NumPy, pandas, SciPy) so the template is ready for numerical and tabular
  data workflows.
...
- I want to use self-hosted multiversion docs via GitHub Pages. I need multiversion support tools from Sphinx that
  integrates well with the PyData Sphinx theme.
